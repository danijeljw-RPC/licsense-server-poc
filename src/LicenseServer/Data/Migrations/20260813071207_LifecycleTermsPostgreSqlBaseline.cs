...
{
    /// <inheritdoc />
    public partial class LifecycleTermsPostgreSqlBaseline : Migration
    {
        /// <inheritdoc />
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            ...
            migrationBuilder.CreateTable(
                ...
                columns: table => new
                {
                    ...
                    ExpirySubMicrosecondTicks = table.Column<int>(type: "integer", nullable: false, defaultValue: 0),
                    ...
                    CancellationReference = table.Column<string>(type: "character varying(200)", maxLength: 200, nullable: true),
                    Version = table.Column<long>(type: "bigint", nullable: false)
                },
                constraints: table =>
                {
                    table.PrimaryKey("PK_Licenses", x => x.Id);
                    table.CheckConstraint("CK_Licenses_ExpiryPrecision", "\"ExpirySubMicrosecondTicks\" BETWEEN 0 AND 9");
                    table.CheckConstraint("CK_Licenses_TerminalState", "NOT (\"CancelledAt\" IS NOT NULL AND \"RevokedAt\" IS NOT NULL)");
                    table.ForeignKey(
            ...
            migrationBuilder.CreateTable(
                ...
                constraints: table =>
                {
                    ...
                    table.CheckConstraint("CK_Entitlements_LicenseType", "\"LicenseType\" IN ('perpetual', 'subscription', 'evaluation')");
                    table.CheckConstraint("CK_Entitlements_Seats", "\"Seats\" > 0");
                    table.ForeignKey(
            ...
            migrationBuilder.Sql("""
                ...
                END;
                $$;
                CREATE TRIGGER "TR_AuditRecords_AppendOnly"
                BEFORE UPDATE OR DELETE ON "AuditRecords"
                FOR EACH ROW EXECUTE FUNCTION prevent_audit_record_mutation();

                CREATE OR REPLACE FUNCTION enforce_license_cancellation_history()
                RETURNS trigger LANGUAGE plpgsql AS $$
                BEGIN
                    IF NEW."CancelledAt" IS NOT NULL AND OLD."CancelledAt" IS NULL
                       AND EXISTS (SELECT 1 FROM "Activations" WHERE "LicenseRecordId" = NEW."Id") THEN
                        RAISE EXCEPTION 'A license with activation history cannot be cancelled';
                    END IF;
                    RETURN NEW;
                END;
                $$;
                CREATE TRIGGER "TR_Licenses_CancellationHistory"
                BEFORE UPDATE ON "Licenses"
                FOR EACH ROW EXECUTE FUNCTION enforce_license_cancellation_history();
                """);
        }
        ...
        {
            migrationBuilder.Sql("""
                DROP TRIGGER IF EXISTS "TR_Licenses_CancellationHistory" ON "Licenses";
                DROP FUNCTION IF EXISTS enforce_license_cancellation_history();
                DROP TRIGGER IF EXISTS "TR_AuditRecords_AppendOnly" ON "AuditRecords";
